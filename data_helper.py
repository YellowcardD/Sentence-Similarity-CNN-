...
def read_data_sets(train_dir):
    ...
    # dictionary for words and word embedding matrix
    global sr_word2id, word_embedding
    sr_word2id, word_embedding = build_glove_dic()

    # word2id, transform word to id in multi-thread way
    # p = Pool()
    # s1 = np.asarray(p.map(seq2id, s1))
    # s2 = np.array(p.map(seq2id, s2))
    # p.close()
    # p.join()
...
def batch_iter(data, batch_size, num_epochs, shuffle=True):
    ...
    for epoch in range(num_epochs):
        ...
        if shuffle:
            ...
            shuffled_data = data_size[shuffle_indices]
        else:
            shuffled_data = data
